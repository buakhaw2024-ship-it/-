...
| `OPENROUTER_MODEL` | | 默认 `nousresearch/hermes-3-llama-3.1-70b`，可换任意 OpenRouter 模型 |
| `OPENROUTER_BASE_URL` | | 默认 `https://openrouter.ai/api/v1`；想直连 DeepSeek 等可改成其 `/v1` |
| `PORT` | | 仅 Node 版，默认 8787 |

换模型示例（更便宜的中文模型）：把 `OPENROUTER_MODEL` 设成
`deepseek/deepseek-chat`、`qwen/qwen-2.5-72b-instruct` 等即可。
